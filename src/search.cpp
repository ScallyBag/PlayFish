...
void MainThread::search() {
  ...
  else
  {
	  ...
	  if (bool(Options["Use Book3"]) && !Limits.infinite && !Limits.mate)
	      bookMove = polybook3.probe(rootPos);

	  if (bool(Options["Use Book4"]) && !Limits.infinite && !Limits.mate)
	      bookMove = polybook4.probe(rootPos);

	  if (bool(Options["Use Book5"]) && !Limits.infinite && !Limits.mate)
	      bookMove = polybook5.probe(rootPos);
	  
	  if (bool(Options["Use Book6"]) && !Limits.infinite && !Limits.mate)
	      bookMove = polybook6.probe(rootPos);
...
moves_loop: // When in check, search starts from here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
      ...
      if ( doLMRStep &&   depth >= 3 //full threads patch
